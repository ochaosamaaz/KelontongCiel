...
import { setupWebCheckout } from './web-checkout.js';
import { registerCustomer, loginCustomer, getCustomer, getCustomerOrders, setResellerStatus, getAllCustomers, getResellerPrice } from './customer-auth.js';
...
function setupRoutes(app, deps) {
    ...
    app.get('/api/store/products', async (req, res) => {
        try {
            ...
            const publicProducts = merged.map(p => ({
                id: p.productId,
                name: p.productName,
                category: (p.category && p.category.toLowerCase() === 'koala') ? 'Premium' : (p.category || 'Digital'),
    ...
    // Public API: Get store config (for landing page - WA number, TG bot, etc)
    app.get('/api/store/config', (req, res) => {
        try {
            const config = getConfig()[0] || {};
            const wa = config.whatsapp || {};
            res.json({
                storeName: config.store_name || 'Kelontong Ciel',
                waNumber: wa.bot_number || config.admin_contact_whatsapp || '6283852212648',
                telegramBot: 'kelontongciel_bot',
                adminWA: config.admin_contact_whatsapp || '6281809182368',
            });
        } catch { res.json({ storeName: 'Kelontong Ciel', waNumber: '6283852212648', telegramBot: 'kelontongciel_bot', adminWA: '6281809182368' }); }
    });

    // ==========================================
    // CUSTOMER AUTH ROUTES (public, no admin auth needed)
    // ==========================================
    ...
    app.post('/api/customer/login', (req, res) => {
        ...
        if (result.success) {
            ...
            req.session.customer = {
                ...
                isReseller: result.customer.isReseller,
                resellerDiscount: result.customer.resellerDiscount || 0,
            };
    ...
    app.get('/api/customer/profile', (req, res) => {
        ...
        const customer = getCustomer(req.session.customer.id);
        if (!customer) {
            delete req.session.customer;
    ...
    app.get('/api/customer/orders', (req, res) => {
        ...
        const orders = getCustomerOrders(req.session.customer.id);
        res.json({ success: true, orders });
    });

    // POST /api/admin/customers/:id/reseller — admin toggles reseller status
    app.post('/api/admin/customers/:id/reseller', (req, res) => {
        // This route is protected by the admin checkAuth middleware (below)
        const { isReseller, discount } = req.body;
        const result = setResellerStatus(req.params.id, isReseller, discount);
        if (result.success) {
            res.json({ success: true, customer: result.customer });
        } else {
            res.status(400).json({ success: false, error: result.error });
    ...
    // GET /api/admin/customers — list all customers for admin
    ...
    app.get('/account', (req, res) => {
        res.sendFile(path.join(projectRoot, 'public', 'account.html'));
    });
    ...
    app.get('/warranty', (req, res) => {
        res.sendFile(path.join(projectRoot, 'public', 'warranty.html'));
    });
            ...
            try {
                ...
                if (fs.existsSync(webTxFile)) {
                    ...
                    webTxs.forEach(tx => {
                        if (tx.status === 'PAID') {
                            totalProfit += Number(tx.profit) || 0;
                            totalRevenue += Number(tx.payAmount) || Number(tx.totalPrice) || 0;
                            totalSales += Number(tx.quantity) || 1;
                        }
                    });
                }
            } catch (e) { console.error('[ROUTES] Web transaction load failed for dashboard:', e.message); }
    ...
    // ==========================================
    // WEB CHECKOUT (Customer-facing QRIS checkout)
    // ==========================================
    setupWebCheckout(app, {
        gopay,
        sendWhatsAppMessage: async (jid, text) => {
            const fn = app.get('sendWhatsAppMessage');
            if (fn) return await fn(jid, text);
        },
        notifyAdmin: app.get('notifyAdmin') || null,
    });
