...
function setupRoutes(app, deps) {
    ...
    async function validateLicense(license) {
        try {
            const apiUrl = `https://whitelist-bot.com/api.php?license=${encodeURIComponent(license)}`;
            ...
            return { valid: false, error: 'Error connecting to license server' };
        }
    }

    // ==========================================
    // LOGIN ROUTES (Must be before checkAuth middleware)
    // ==========================================
    ...
    app.post('/api/login', async (req, res) => {
        ...
        if (result.valid) {
            ...
            console.log(`   -> License: ${result.user.license}`);
            console.log(`   -> Full Name: ${result.user.fullName}`);
            console.log(`   -> Created: ${result.user.created}`);
            console.log(`   -> Duration: ${result.user.duration} days`);
