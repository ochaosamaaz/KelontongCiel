...
_Scan QR di atas dan bayar sesuai total._`;
        ...
        try {
            ...
            waSock.ev.on('connection.update', async (update) => {
                ...
                if (connection === 'close') {
                    ...
                    const isConnectionTerminated = code === 428;
                    const isFatal = isBanned || isLoggedOut || isBadMAC || isConnectionTerminated;
                    ...
                    if (isFatal) {
                        ...
                        } else if (isConnectionTerminated) {
                            waLastError = 'connection_terminated';
                            console.log('[WA] Connection Terminated (428). Session cleared. Waiting 30s before retry...');
                            io.emit('wa-error', { type: 'logged_out', message: 'Sesi WhatsApp expired. Menunggu 30 detik lalu generate QR baru...' });
                            // Wait 30s then try fresh connection (will generate new QR)
                        ...
                        // Do NOT reconnect immediately for fatal errors (428 has its own delayed retry above)
                    } else {
    ...
    app.get('/api/all-transactions', (req, res) => {
        try {
                ...
                if (fs.existsSync(webTxFile)) {
                    webTxns = JSON.parse(fs.readFileSync(webTxFile, 'utf8'))
                        .map(tx => ({
                            ...
                            product: tx.productName || '-',
                            quantity: tx.quantity || 1,
                            amount: tx.payAmount || tx.totalPrice || 0,
                            profit: tx.profit || 0,
                            status: tx.status || 'UNKNOWN',
            ...
            const all = [...normalizedTg, ...normalizedWa, ...webTxns].sort((a, b) => b.timestamp - a.timestamp);
