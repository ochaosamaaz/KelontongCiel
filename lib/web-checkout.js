...
import { getCustomer, getResellerPrice } from './customer-auth.js';
...
// ==========================================
// Module-level reference to notifyAdmin (set during setup)
let _notifyAdmin = null;
let _app = null;

function setupWebCheckout(app, deps) {
    const { gopay, sendWhatsAppMessage, notifyAdmin: notifyAdminFn } = deps;
    _app = app;
    _notifyAdmin = notifyAdminFn;

    // Helper: get notifyAdmin (might be registered later)
    const getNotifyAdmin = () => _notifyAdmin || app.get('notifyAdmin');
    ...
    app.post('/api/web/checkout', async (req, res) => {
        ...
        // Check customer session (optional — guest checkout still works)
        let customerId = null;
        let customerSession = req.session && req.session.customer;
        ...
        try {
            ...
            let totalPrice = bulk.totalPrice;

            // Apply reseller discount if customer is a reseller
            if (customerId && customerSession && customerSession.isReseller) {
                const customer = getCustomer(customerId);
                if (customer && customer.isReseller) {
                    totalPrice = getResellerPrice(totalPrice, customer);
                }
            }
            ...
            try {
                const notify = getNotifyAdmin();
                if (notify) notify({ type: 'new', platform: '🌐 Website', buyer: waNumber, product: product.productName, quantity: qty, amount: paymentResult.paymentRupiah, invoice: orderId });
            } catch (e) {}
...
async function processDelivery(orderId, gopayConfig, sendWhatsAppMessage) {
            ...
            if (ks.is_active && ks.api_key) {
                ...
                if (product && product.variant_code) {
                    ...
                    if (checkoutResult.success) {
                        // Extract account credentials from KoalaStore response
                        const rawData = checkoutResult.data || checkoutResult;
                        let accounts = [];
                        
                        // Handle various response formats from KoalaStore
                        if (rawData.items && Array.isArray(rawData.items)) {
                            accounts = rawData.items.map(item => item.credential || item.account || item.serial_number || JSON.stringify(item));
                        } else if (rawData.credentials && Array.isArray(rawData.credentials)) {
                            accounts = rawData.credentials;
                        } else if (rawData.credential) {
                            accounts = [rawData.credential];
                        } else if (rawData.account) {
                            accounts = [rawData.account];
    ...
    // Notify admin: paid
    try {
        const notify = _notifyAdmin || (_app ? _app.get('notifyAdmin') : null);
        if (notify) notify({ type: 'paid', platform: '🌐 Website', buyer: order.customerWA, product: order.productName, quantity: order.quantity, amount: order.payAmount, invoice: orderId, status: 'PAID' });
    } catch (e) {}
}
...
async function sendAccountToWA(order, deliveryData, sendWhatsAppMessage) {
    ...
    invoiceMsg += `Support: wa.me/${order.customerWA ? '6281809182368' : '6281809182368'}`;
